...
class RuleEngine:
                    ...
                    for rule in rules:
                        ...
                        if trace:
                            ...
                            if action:
                                ...
                                self._update_attribute_changes_tracker(consequences, iteration_attribute_changes, rule)
    ...
    def _trace_rule_evaluation(
        self,
        rule: Rule,
        resolved_facts: list[Fact],
        rule_result: bool,
        action,
        rule_timestamp: datetime,
        elapsed: float,
        consequences: list,
        ...
        context = self._extract_context_from_evaluation_and_consequences(rule.when, resolved_facts, consequences)
            ...
            if "lambda:" in source:
                ...
                for fact_ref in condition.facts:
                    ...
                    if class_name in fact_map:
                        ...
                        formatted_expr = formatted_expr.replace(f"{class_name}.{attr_name}", replacement)
    ...
    def _format_compound_condition(self, condition, fact_map: dict, result: bool) -> str:
        ...
        operator_map = {Operator.AND: "and", Operator.OR: "or", Operator.XOR: "xor"}
    ...
    def _check_for_rule_ordering_warnings(
        self, consequences: list, attribute_changes: dict, current_rule: Rule
    ) -> list[str]:
        """
    ...
    def _update_attribute_changes_tracker(self, consequences: list, attribute_changes: dict, current_rule: Rule):
        """
    ...
    def _extract_context_from_evaluation_and_consequences(
        self, condition, resolved_facts: list[Fact], consequences: list
    ) -> list:
        """
